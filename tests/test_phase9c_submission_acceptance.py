...
def _clean_code_root(tmp_path: Path):
    root=tmp_path/'code_root'; root.mkdir()
    (root/'README.md').write_text('test code root', encoding='utf-8')
...
def test_dry_run_code_inventory_matches_real_code_inventory(tmp_path):
    ...
    code_root=_clean_code_root(tmp_path)
    buf=io.StringIO()
    with contextlib.redirect_stdout(buf):
        build_submission_bundle.main(['--code-root',str(code_root), '--ner-model',str(ner),'--assertion-model',str(assertion),'--rxnorm-kb',str(kb),'--rxnorm-dense-index',str(dense),'--bge-model',str(bge),'--output',str(tmp_path/'dry'),'--dry-run'])
