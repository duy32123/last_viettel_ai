...
def main(argv=None):
    ...
    cfg={'submission_mode':True,'production':True,'runtime':{'device':'auto','low_vram_mode':True},'ner':{'model_path':str(root/manifest['ner']['path']),'mock':False},'assertion':{'model_path':str(root/manifest['assertion']['path']),'thresholds_path':str(root/manifest['assertion']['thresholds']),'mock':False},'rxnorm':{'kb_dir':str(root/manifest['rxnorm']['path']),'dense_index_dir':str(root/manifest['dense_index']['path']),'bge_model_path':str(root/manifest['bge']['path']),'retrieval_mode':'bge_dense','include_unverified':False,'strict':True,'use_reranker':False,'top_k':10},'icd10':{'require':False},'diagnostics':{'report_path':str(report_dir/'submission_run_report.json')},'official_evaluation':False}
