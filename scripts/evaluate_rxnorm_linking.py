...
from src.linking.dense import BGEM3Backend, BGERerankerBackend, DenseAliasIndex, MockDenseEncoder, MockReranker, kb_checksum, load_dense_index, metrics_from_ranks, rrf_fuse, tune_rrf, rerank_candidates, tune_blend
...
def main(argv=None):
    ...
    include_unverified=bool(cfg.get('include_unverified',False)); top_k=int(cfg.get('top_k',20)); expected_checksum=kb_paths_checksum(paths); candidate_universe=len(verified_codes)
    bm25_called=dense_called=rerank_called=False; model_name=model_revision=device=None; cache_validation=None
    lexical=None
    def bm25_rank(e):
        nonlocal bm25_called; bm25_called=True
        return [c.code for c in lexical.search(e.get('context') or e.get('mention'), 'THUỐC', top_k=candidate_universe, use_fuzzy=False)]
    dense_index=encoder=None; qvec_cache={}
    def load_dense():
        ...
        expected={'model_name':model_name,'model_revision':model_revision,'kb_checksum':kb_checksum(paths),'normalization':'l2','include_unverified':include_unverified,'max_length':int(cfg.get('max_length',8192))}
    ...
    def ensure_qvecs():
        ...
        for sp,examples in splits.items():
            ...
            vecs,_=_encode_query_vectors(encoder,texts,batch_size=int(cfg.get('batch_size',16)),expected_dim=len(dense_index.vectors[0]) if dense_index.vectors else 0)
            for e,v in zip(examples,vecs): qvec_cache[id(e)]=v
    def dense_rank(e):
        nonlocal dense_called; dense_called=True; ensure_qvecs()
        return [r['code'] for r in dense_index.search_vector(qvec_cache[id(e)], top_k=candidate_universe)]
    ...
    if requested=='bm25':
        ...
        executed='hybrid_rrf'; load_dense(); ensure_qvecs(); params=tune_rrf(splits.get('dev',[]), bm25_rank, dense_rank)
        ranker=lambda e: rrf_fuse([(params['bm25_weight'],bm25_rank(e)),(params['dense_weight'],dense_rank(e))], params['rrf_k'], top_k=candidate_universe)
    else:
        executed='reranker'; load_dense(); ensure_qvecs(); params=tune_rrf(splits.get('dev',[]), bm25_rank, dense_rank)
        base_rank=lambda e: rrf_fuse([(params['bm25_weight'],bm25_rank(e)),(params['dense_weight'],dense_rank(e))], params['rrf_k'], top_k=candidate_universe)
        ...
        def rerank_codes(e):
            ...
            rows=rows_for(e); out=rerank_candidates(e.get('context') or e.get('mention'), rows, reranker, batch_size=int(cfg.get('reranker_batch_size',16)))
        ...
        fallback=dev_rerank.get('recall@1',0)<dev_base.get('recall@1',0) and dev_rerank.get('mrr',0)<dev_base.get('mrr',0)
