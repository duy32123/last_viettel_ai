...
def load_rxnorm_codes(rxnorm_kb=None):
    ...
    return verified
def validate_file_pair(input_txt, output_json, rxnorm_codes=None):
...
def validate(input_dir, output_dir, expected_count=None, rxnorm_kb=None):
    ...
    total=0
    for inp in inputs:
        total += validate_file_pair(inp, Path(output_dir)/(inp.stem+'.json'), rxnorm_codes)['concept_count']
    return {'valid':True,'input_count':len(inputs),'output_count':len(outputs),'concept_count':total,'validated_rxcui_count':len(rxnorm_codes or [])}
def main(argv=None):
