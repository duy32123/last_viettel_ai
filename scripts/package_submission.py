...
def sha256(path):
    ...
    return h.hexdigest()
def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument('--input-dir',required=True); p.add_argument('--output-dir',required=True); p.add_argument('--zip-path',required=True); p.add_argument('--expected-count',type=int); p.add_argument('--rxnorm-kb', required=True); p.add_argument('--icd10-kb')
    ns=p.parse_args(argv); report=validate(ns.input_dir, ns.output_dir, ns.expected_count, ns.rxnorm_kb, ns.icd10_kb)
