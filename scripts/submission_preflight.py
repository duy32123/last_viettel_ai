...
def main(argv=None):
    ...
    report['checks'].update({'rxnorm_verified':True,'candidate_universe':universe,'dense_manifest':str(ddir/'dense_manifest.json'),'bge_local_snapshot':True,'no_auxiliary_icd':True,'rxnorm_release':records[0].version if records else None,'dense_dimension':dmanifest.get('dimension')})
    if ns.deep:
