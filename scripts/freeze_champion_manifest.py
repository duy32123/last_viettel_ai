...
def build_manifest(args):
    return {
        ...
        'low_vram_mode':True,
        'icd10':{'production_blocker':'official ICD-10 KB missing; diagnosis candidates suppressed','auxiliary_icd_allowed_in_production':False},
...
def main(argv=None):
    p=argparse.ArgumentParser(); p.add_argument('--ner-model',required=True); p.add_argument('--assertion-model',required=True); p.add_argument('--rxnorm-kb',required=True); p.add_argument('--rxnorm-dense-index',required=True); p.add_argument('--bge-model',required=True); p.add_argument('--output',default='configs/champion.phase9.json'); p.add_argument('--candidate-universe',default='62099'); p.add_argument('--rxnorm-release',default='2026-07-06'); p.add_argument('--bge-revision',default='main')
